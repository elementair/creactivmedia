   ...
   <div class="col-md-12 col-lg-12">
    ...
    if($msj1==TRUE){
      ...
      <div class="alert alert-danger" role="alert" id="error1" style="display:none;">
        ...
        Agregue un nombre para el perfil, no puede quedar vacio.
      </div>
      <?php 
    }
    ?>
    ...
    if($msj2==TRUE){
      ...
      <div class="alert alert-danger" role="alert" id="error2" style="display:none;">
        <span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
        <span class="sr-only">Error:</span>
        Agregue un puesto para el perfil.
      </div>
      <?php 
    }
    ?>
    ...
    if($msj3==TRUE){
      ...
      <div class="alert alert-danger" role="alert" id="error3" style="display:none;">
        <span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
        <span class="sr-only">Error:</span>
        Seleccione un permiso valido en la lista desplegable
      </div>
      <?php 
    }
    ?>
    ...
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
              ...
              <?php
              if ($sql = mysqli_query($con,"SELECT id, user, nombre, activo, permiso, img, puesto, bio, playlist, fecha_alta FROM user WHERE id='".$_GET["id"]."' ORDER BY id DESC")) {
                ...
                if(mysqli_num_rows($sql) == 0){
                  ...
                  while($row0 = mysqli_fetch_assoc($sql)) {
                   ...
                   $status1=($row0["activo"]==0) ? 'selected' : '';
                   $status2=($row0["activo"]==1) ? 'selected' : '';
                   $permiso1=($row0["permiso"]==1) ? 'selected' : '';
                   $permiso2=($row0["permiso"]==2) ? 'selected' : '';
                  ...
                  //$variable = ($haceFrio) ? "Hace frío" : "No hace frío";
                   echo "<br><a href='usuarios.php' class='btn btn-default' role='button'>Regresar</a><br>";
                    ...
                    echo "
                    <form action='procesa-editar-usuario.php' method='POST' enctype='multipart/form-data' role='form'>
                        <legend><strong>".$row0["nombre"]."</strong></legend>

                        <input type='hidden' name='id' value='".$_GET["id"]."'>

                        <div class='form-group'>
                        <label for=''>Usuario</label>
                        <input type='text' class='form-control' id='' value='".$row0["user"]."' placeholder='usuario' disabled>
                        </div>

                        <div class='form-group'>
                        <label for=''>Nombre</label>
                        <input type='text' class='form-control' id='' name='nombre' value='".$row0["nombre"]."' placeholder='Nombre usuario'>
                        </div>

                        <div class='form-group'>
                        <label for=''>Puesto</label>
                        <input type='text' class='form-control' id='' name='puesto' value='".$row0["puesto"]."' placeholder='Puesto'>
                        </div>";
...
/*
                       ...
                       echo " <div class='form-group'>
                        ...
                        <select name='activo' id='input' class='form-control' required='required'>";
                    

                    echo "<option value='0' ".$status1.">Desactivado</option>";
                    echo "<option value='1' ".$status2.">Activo</option>";
...
*/
                    ...
                    echo "<div class='form-group'>
                        ...
                        <select name='permiso' id='input' class='form-control' required='required'>";
                    

                    echo "<option value='1' ".$permiso1.">Editor / Administrador</option>";
                    echo "<option value='2' ".$permiso2.">Editor</option>";
                    ...
                    echo "</select>
                        ...
                        <div class='form-group'>
                        <label for=''>Biograf&iacute;a</label>
                        <textarea id='input' class='form-control' name='bio' rows='3' required='required'>".$row0["bio"]."</textarea>
                        </div>


                        <div class='form-group'>
                        <label for=''>Playlist</label>
                        <input type='text' class='form-control' id='' name='playlist' value='".$row0["playlist"]."' placeholder='Input field'>
                        </div>
