...
if(!class_exists('FPD_Admin_Ajax')) {
	...
	class FPD_Admin_Ajax {
		...
		public function new_design_category() {
			...
			if( is_wp_error($result) ) {
				...
				echo json_encode(array(
					...
					'html' => FPD_Admin_Fancy_Designs::get_category_item_html($result['term_id'], $_POST['title'])
				));
		...
		public function load_view() {
			...
			$fancy_view = new Fancy_View((int) $_POST['view_id']);
		...
		public function new_product() {
			...
			if ( class_exists( 'Fancy_Product' ) ) {
				...
				$id = Fancy_Product::create( $_POST['title'], $options, $thumbnail );
				...
				echo json_encode(
					array(
						...
						'html' => FPD_Admin_Manage_Fancy_Products::get_product_item_html( $id, $_POST['title'], '', $options, $thumbnail )
					)
				);
		...
		public function edit_product() {
			...
			if ( class_exists( 'Fancy_Product' ) ) {
				...
				$columns = $fancy_product->update(
					...
					isset($_POST['thumbnail']) ? $_POST['thumbnail'] : false
				);
		...
		public function duplicate_product() {
			...
			$source_product = new Fancy_Product( $source_id );
		...
		public function save_as_template() {
			...
			foreach($views as $view) {
				...
				unset($view->options);
				$view->elements = @unserialize($view->elements) ? unserialize($view->elements) : $view->elements;
			}
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="'.$width.'" height="'.$height.'" xml:space="preserve">'.$svg.'</svg>';
		...
		public function export_product() {
		    ...
		    foreach($views as $view) {
				...
				$elements = @unserialize($view->elements) ? unserialize($view->elements) : fpd_convert_obj_string_to_array($view->elements);
				...
				$final_view['options'] = $view->options;
			...
			fwrite($fop, json_encode($final_json, JSON_PRETTY_PRINT));
			fclose($fop);
