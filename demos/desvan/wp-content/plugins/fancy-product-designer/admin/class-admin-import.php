...
if( !class_exists('FPD_Admin_Import') ) {
	...
	class FPD_Admin_Import {
		...
		private function read_json( $dir ) {
			...
			$fp_id = Fancy_Product::create(
				$json_content->title,
				htmlspecialchars_decode($json_content->options),
				$this->import_image( $json_content->thumbnail )
			);

			if( $fp_id !== false ) {

				$fp = new Fancy_Product($fp_id);
		...
		private function import_image( $image ) {
			...
			if( $this->add_to_media_library ) {
				...
				if ( is_wp_error( $file_array['tmp_name'] ) ) {
					$this->output_error($image. __(': Image could not be added to media library.', 'radykal'));
					return '';
				}
