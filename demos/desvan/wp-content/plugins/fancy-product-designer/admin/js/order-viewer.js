		...
		if(	isReady && !loadingProduct ) {
			...
			complete: function(data) {
				...
				if(data == undefined || data.responseJSON) {
					...
					fpdLoadOrder(JSON.parse(data.responseJSON.order_data), bulkVariations);
		...
		if(_checkAPI()) {
			...
			if(stage.getActiveObject()) {
				...
				stage.setBackgroundColor(backgroundColor, function() {
					...
					var width_orig = element.width/2,
						height_orig = element.height/2,
						paddingTemp = element.padding;
					...
					if(element.clippingRect) {
						...
						if(!jQuery('#fpd-without-bounding-box').is(':checked')) {
							...
							if(jQuery('#fpd-restore-oring-size').is(':checked')) {
								dataURLOpts.left = width_orig - ((width_orig * element.scaleX ) - (element.clippingRect.left - elementPoint.x )) * scaleXTo1 ;
								dataURLOpts.top = height_orig - ((height_orig * element.scaleY ) - (element.clippingRect.top - elementPoint.y )) * scaleYTo1 ;
