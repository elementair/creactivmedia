jQuery(document).ready(function($) {
	...
	if($().select2) {
		...
		.on("change", function (e) {
			...
			var $this = $(this);
			if($this.nextAll('.fpd-select2-input:first').length > 0) {
		...
		$('.radykal-select2').each(function(i, select2) {
			...
			var $select2 = $(select2);
			if($select2.is('select') && $select2.nextAll('.fpd-select2-input:first').length > 0) {
	...
	$('input[data-relation]').change(function() {
		...
		for (var key in relationObj) {
			...
			if (relationObj.hasOwnProperty(key)) {
				...
				$target.parents('tr:first').toggle(value);
